...
def train(args, model, device, xs, ys, optimizer,criterion, taskcla):
    for k, ncla in taskcla:
        ...
        for i in range(0,len(r),args.batch_size_train):
            ...
            optimizer.zero_grad()
            output = model(data)
            loss = criterion(output[k], target)
            loss.backward()
            optimizer.step()

def val(args, model, device, xs, ys, criterion, taskcla):
    accs = []
    final_losss = []
    for k, ncla in taskcla:
        x = xs[k]
        ...
        with torch.no_grad():
            for i in range(0, len(r), args.batch_size_train):
                ...
                output = model(data)
                loss = criterion(output[k], target)
                pred = output[k].argmax(dim=1, keepdim=True)
        ...
        accs.append(acc)
        final_losss.append(final_loss)
    return np.mean(final_losss), np.mean(accs)
...
def main(args):
    ...
    ys_test = []
    for k, ncla in taskcla:
        xs_train.append(data[k]['train']['x'])
        ys_train.append(data[k]['train']['y'])
        xs_val.append(data[k]['valid']['x'])
        ys_val.append(data[k]['valid']['y'])
        xs_test.append(data[k]['test']['x'])
        ys_test.append(data[k]['test']['y'])
    ...
    for epoch in range(1, args.n_epochs+1):
        # Train
        clock0=time.time()
        train(args, model, device, xs_train, ys_train, optimizer, criterion, taskcla)
        clock1=time.time()
        tr_loss,tr_acc = val(args, model, device, xs_train, ys_train,  criterion, taskcla)
        log.info('Epoch {:3d} | Train: loss={:.3f}, acc={:5.1f}% | time={:5.1f}ms |'.format(epoch,\
                                                    tr_loss,tr_acc, 1000*(clock1-clock0)))
        # Validate
        valid_loss,valid_acc = val(args, model, device,  xs_val, ys_val,  criterion, taskcla)
    ...
    # Test
    log.info ('-'*40)
    test_loss, test_acc = val(args, model, device, xs_test, ys_test,  criterion, taskcla)
    ...
    torch.save(best_model, args.savename + '/' + str_time + '/model_random_' + str(args.seed) + '.pkl')

    # save accuracy
    jj = 0
    for ii in range(len(taskcla)):
        xtest = xs_test[ii]
        ytest = ys_test[ii]
        _, acc_matrix[task_id,jj] = test(args, model, device, xtest, ytest,criterion,ii)
        jj +=1
    log.info('Accuracies =')
    for i_a in range(len(taskcla)):
        # log.info('\t')
        acc_ = ''
    ...
    log.info ('Final Avg Accuracy: {:5.2f}%'.format(acc_matrix[0].mean()))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--savename', type=str, default='./log/MTL/',
                        help='save path')
