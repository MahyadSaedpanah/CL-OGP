...
def update_space(net, x, y, task_id, device, optimizer, criterion, rest_space=None, space=None, args=None):
    # thresholds = [0.97, 0.97, 0.97, 0.99, 0.99]
    thresholds = [args.gpm_thro] * 5
    log.info('thresholds: ' +str(thresholds))
    # space_thresholds = [0.95, 0.95, 0.95, 0.9, 0.9]
    space_thresholds = [thresholds[0]-0.02, thresholds[1]-0.02, thresholds[2]-0.02, thresholds[3]-0.02, thresholds[4]-0.02]
    log.info('space_thresholds: ' + str(space_thresholds))
                ...
                if valid_loss<best_loss:
                    ...
                    if patience<=0:
                        ...
                        if lr <= 5e-4 and up == True and count < 2:
                            ...
                            rest, up, space = update_space(model, xtrain, ytrain, task_id, device, optimizer, criterion, rest, space, args)
